...
int nCoinbaseMaturityMultipiler = 300;
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    int64 nActualSpacing = pindexPrev->GetBlockTime() - pindexPrevPrev->GetBlockTime();
        if (nBestHeight >= 305052 && nActualSpacing < 0) nActualSpacing = 0; // patch to prevent negative nActualSpacing
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "GrowthCoin", CClientUIInterface::MSG_WARNING);
        StartShutdown();
        return false;
    }
    return true;
